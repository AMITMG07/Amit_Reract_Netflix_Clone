...
function Watchlist() {
  ...
  return (
    <>
      {/* Header: Netflix Logo + Home Button */}
      <div className="flex justify-between items-center px-4 sm:px-6 lg:px-10 py-5 space-x-4">
        <svg
            viewBox="0 0 111 30"
            version="1.1"
        ...
        <Link
          to="/"
          className="border border-white text-white px-5 py-2 rounded-md hover:bg-white hover:text-black transition text-sm sm:text-base"
        >
          Home
        </Link>
      </div>
      ...
      <div className="mt-12 px-4 sm:px-6 lg:px-10 w-full max-w-screen-xl mx-auto">
        {wishlist.length === 0 ? (
          <p className="text-white text-lg text-center">No movies in your watchlist.</p>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6 md:gap-8">
            {wishlist.map((movie, idx) => (
              <div key={idx} className="relative group">
                ...
                {/* Remove Button in Bottom-Left */}
                <button
                  onClick={() => removeFromWishlist(movie.slug)}
                  className="absolute bottom-2 left-2 bg-red-600 text-white text-xs px-3 py-1 rounded hover:bg-red-700 transition shadow-md"
                >
                  Remove
                </button>
              </div>
